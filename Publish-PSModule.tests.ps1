...
"@
        ...
        It "Initial Build" {
            { & $ScriptPath\Publish-PSModule.ps1 -Path $ScriptPath\Test-Module -Verbose } | Should Not Throw
        }
        It "Manifest exists" {
...
"@
        $Module | Out-File $ScriptPath\Test-Module\Source\Public\PublicFunction.ps1
        It "Duplicate Function Name Build" {
            { & $ScriptPath\Publish-PSModule.ps1 -Path $ScriptPath\Test-Module -Verbose } | Should Throw
        }
    }
}
